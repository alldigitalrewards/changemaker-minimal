export function renderInviteEmail(args: {
  ...
  <div style="font-family: system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial; line-height:1.6; color:#111">
    ...
    <p>${args.inviterEmail} invited you to join <strong>${args.workspaceName}</strong>${args.challengeTitle ? ` for the challenge <strong>${args.challengeTitle}</strong>` : ''} as <strong>${args.role}</strong>.</p>
