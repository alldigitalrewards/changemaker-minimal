 ...
 * Populates staging database with realistic demo data including:
 ...
 * - Leaderboard data with historical points
 */
...
const adminUsers = [
  {
    ...
    workspaceMemberships: [
      { workspace: "alldigitalrewards", isPrimary: true, role: 'ADMIN' as Role },
      { workspace: "acme", isPrimary: false, role: 'ADMIN' as Role },
      { workspace: "sharecare", isPrimary: false, role: 'PARTICIPANT' as Role },
    ],
  },
  {
    ...
    workspaceMemberships: [
      { workspace: "alldigitalrewards", isPrimary: true, role: 'ADMIN' as Role },
      { workspace: "acme", isPrimary: false, role: 'MANAGER' as Role },
    ],
  },
  {
  ...
  },
  {
    ...
    workspaceMemberships: [
      { workspace: "alldigitalrewards", isPrimary: true, role: 'ADMIN' as Role },
      { workspace: "sharecare", isPrimary: false, role: 'ADMIN' as Role },
    ],
  },
];
...
/**
 * Seed the database with realistic demo data for staging environment.
 * The routine is largely idempotent via upserts.
 */
async function seed() {
  try {
    console.log("🌱 Starting comprehensive staging seed...\n");
    ...
    for (const workspace of workspaces) {
      ...
      console.log(
        `✓ Workspace ready: ${workspace.name} (tenant: ${workspace.slug})`,
      );
    }
    ...
    for (const admin of adminUsers) {
      ...
      if (supabaseUser) {
        ...
        // Create WorkspaceMemberships with specified roles
        for (const membership of admin.workspaceMemberships) {
          ...
          if (workspace) {
            ...
            console.log(
              `  ✓ Added ${admin.email} to ${workspace.name} as ${membership.role}${membership.isPrimary ? " (primary)" : ""}`,
            );
          }
        }
        console.log(
    ...
    if (firstAdmin) {
      ...
      for (const workspace of createdWorkspaces) {
        ...
        const inviteTemplate = await prisma.workspaceEmailTemplate.findFirst({
          where: {
            workspaceId: workspace.id,
            type: "INVITE",
          },
        ...
        const enrollmentTemplate = await prisma.workspaceEmailTemplate.findFirst({
          where: {
            workspaceId: workspace.id,
        ...
        const activeSegment = await prisma.workspaceParticipantSegment.findFirst({
          where: {
            workspaceId: workspace.id,
        ...
        const highAchieversSegment = await prisma.workspaceParticipantSegment.findFirst({
          where: {
            workspaceId: workspace.id,
            name: "High Achievers",
          },
      ...
      for (const workspace of createdWorkspaces) {
        ...
        const generalInvite = await prisma.inviteCode.findFirst({
          where: {
            workspaceId: workspace.id,
        ...
        const vipInvite = await prisma.inviteCode.findFirst({
          where: {
            workspaceId: workspace.id,
    ...
    // Create manager users with single workspace membership
    ...
    for (const manager of managerUsers) {
      ...
      if (supabaseUser) {
        ...
        // Create WorkspaceMembership with MANAGER role
        if (workspace) {
          await prisma.workspaceMembership.upsert({
            where: {
              userId_workspaceId: {
    ...
    // Historical date range: August 1st, 2025 to present
    const historicalStartDate = new Date('2025-08-01T00:00:00Z');
    const now = new Date();
    const daysSinceHistoricalStart = Math.floor((now.getTime() - historicalStartDate.getTime()) / (24 * 60 * 60 * 1000));

    for (const workspace of createdWorkspaces) {
      // Create 5 challenges per workspace spanning the historical period
      const numChallenges = 5;
      for (let i = 0; i < numChallenges; i++) {
        const template = challengeTemplates[i % challengeTemplates.length];

        // Spread challenges across the historical period (Aug 1 to now)
        const challengeStartOffset = Math.floor((daysSinceHistoricalStart / numChallenges) * i);
        const startDate = new Date(historicalStartDate.getTime() + (challengeStartOffset * 24 * 60 * 60 * 1000));
        const endDate = new Date(startDate.getTime() + (45 * 24 * 60 * 60 * 1000)); // 45 day duration
        const enrollmentDeadline = new Date(startDate.getTime() - 3 * 24 * 60 * 60 * 1000); // 3 days before start
    ...
    for (const workspace of createdWorkspaces) {
      ...
      for (const challenge of workspaceChallenges.slice(0, 2)) {
        const existingBudget = await prisma.challengePointsBudget.findFirst({
    ...
    // Generate comprehensive historical time periods from Aug 1, 2025 to today
    const totalHistoricalDays = daysSinceHistoricalStart;
    const timePeriods = [];

    // Create submission dates spanning the entire historical period
    // More recent activity (weighted towards present for realistic usage patterns)
    for (let i = 0; i < 50; i++) {
      // Exponential distribution favoring recent dates
      const randomFactor = Math.pow(Math.random(), 2); // Square for heavier weighting towards 0
      const daysAgo = Math.floor(randomFactor * totalHistoricalDays);
      const label = daysAgo === 0 ? 'today' :
                    daysAgo <= 1 ? 'yesterday' :
                    daysAgo <= 7 ? 'this week' :
                    daysAgo <= 30 ? 'this month' :
                    daysAgo <= 60 ? 'past 2 months' :
                    'older';
      timePeriods.push({ name: label, daysAgo });
    }

    // Sort by most recent first for predictable seeding
    timePeriods.sort((a, b) => a.daysAgo - b.daysAgo);
    ...
    for (const enrollment of enrolledEnrollments) {
      ...
      // Create 5-12 submissions per enrolled user spread across time periods
      const numSubmissions = 5 + (submissionCount % 8); // 5-12 submissions per participant
      for (let i = 0; i < Math.min(numSubmissions, activities.length); i++) {
        const activity = activities[i];
        if (!activity) continue;

        // Select time period for this submission (cycle through all periods)
    ...
    for (const membership of participantMemberships) {
      ...
      if (membership.workspaceId) {
        // More varied points for realistic leaderboard distribution (5-100 points)
        const participantIndex = participantMemberships.indexOf(membership);
        const pointsPattern = [5, 15, 25, 35, 50, 65, 80, 100];
        const pointsToAward = pointsPattern[participantIndex % pointsPattern.length];
        if (pointsToAward > 0) {
          try {
            await awardPointsWithBudget({
    ...
    for (const workspace of createdWorkspaces) {
      ...
      const existingWorkspaceComm = await prisma.workspaceCommunication.findFirst({
        where: {
          workspaceId: workspace.id,
          scope: "WORKSPACE",
      ...
      for (const challenge of workspaceChallenges.slice(0, 2)) {
        const existingChallengeComm = await prisma.workspaceCommunication.findFirst({
          where: {
            workspaceId: workspace.id,
            challengeId: challenge.id,
            scope: "CHALLENGE",
        ...
        if (challengeActivity) {
          const existingActivityComm = await prisma.workspaceCommunication.findFirst({
            where: {
              workspaceId: workspace.id,
              challengeId: challenge.id,
    ...
    // Print summary
    console.log("\n✨ Comprehensive staging seed completed successfully!\n");
    console.log("📊 Summary:");
    ...
    console.log(
      `  - Users: ${finalUserCount} (${adminUsers.length} admins, ${managerUsers.length} managers, ${participantUsers.length} participants)`,
    );
    ...
    console.log(`    - krobinson@alldigitalrewards.com (3 workspaces: ADMIN, ADMIN, PARTICIPANT)`);
    console.log(`    - jfelke@alldigitalrewards.com (2 workspaces: both ADMIN)`);
    console.log(`    - jhoughtelin@alldigitalrewards.com (2 workspaces: both ADMIN)`);
    console.log(`    - kfelke@alldigitalrewards.com (2 workspaces: ADMIN, MANAGER)`);
    console.log("\n  Manager accounts:");
...
// Run the seed
seed().catch((error) => {
  console.error("Fatal error during staging seeding:", error);
  process.exit(1);
});
