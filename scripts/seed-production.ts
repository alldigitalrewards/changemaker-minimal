...
async function seedProduction() {
  try {
    console.log('🌱 Starting production seed for multi-workspace memberships...\n')
    ...
    for (const config of multiWorkspaceUsers) {
      ...
      if (!authUser) {
        console.log(`  ⚠️  Skipping ${config.email} - no Supabase auth user found`)
        continue
      }
      ...
      if (!user) {
        ...
        const primaryWorkspaceId = workspaces.find(w => w.slug === config.primaryWorkspace)?.id
      ...
      for (const workspaceSlug of config.workspaces) {
        ...
        try {
          ...
          console.log(`  ✓ Added ${config.email} to ${workspace.name}${workspaceSlug === config.primaryWorkspace ? ' (primary)' : ''}`)
        } catch (error) {
          console.error(`  ❌ Error adding membership for ${config.email} to ${workspace.name}:`, error)
        }
      }
    }
    ...
    for (const config of multiWorkspaceUsers) {
      ...
      if (user) {
        console.log(`  ${user.email}: ${user.workspaceMemberships.length} workspace(s)`)
        user.workspaceMemberships.forEach(m => {
          console.log(`    - ${m.workspace.name}${m.isPrimary ? ' (primary)' : ''}`)
        })
      }
    }
